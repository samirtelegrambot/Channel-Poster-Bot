...
def is_admin(user_id):
    return str(user_id) in load_admins() or user_id == OWNER_ID
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "➕ Add Channel":
        await update.message.reply_text("Send the @channelusername or private channel ID (e.g. -1001234567890):")
        return ADD_CHANNEL
        ...
        channels = load_channels().get(str(user_id), [])
        if not channels:
